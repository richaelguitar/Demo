 ...
 * 主播视频页面，连麦功能
 */
public class AnchorVideoUI extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ZGVideoCommunicationHelper.sharedInstance().setmAnchorCallback( new ZGVideoCommunicationHelper.ZGVideoAnchorLiveHelperCallback(){

            @Override
            public TextureView addRenderViewByStreamAdd(ZegoStreamInfo listStream) {
                TextureView playRenderView = AnchorVideoUI.this.mVideoLayoutModel.addStreamToViewInLayout(listStream.streamID);
                AnchorVideoUI.this.playStreamids.add(listStream.streamID);
            ...
            public void onLoginRoomFailed(int errorcode){
                ...
                if(ZGVideoCommunicationHelper.ZGVideoCommunicationHelperCallback.NUMBER_OF_PEOPLE_EXCEED_LIMIT == errorcode){
                    Toast.makeText(AnchorVideoUI.this, "房间已满人，目前demo只展示12人通讯", Toast.LENGTH_LONG).show();
                }else {
                    Toast.makeText(AnchorVideoUI.this, "登录房间失败，请检查网络", Toast.LENGTH_LONG).show();
                }
                AnchorVideoUI.this.onBackPressed();
            ...
            public void onPublishStreamFailed(int errorcode){
                ...
                Toast.makeText(AnchorVideoUI.this, "开启视频通话失败，检查网络", Toast.LENGTH_LONG).show();
                onBackPressed();

            }

            @Override
            public void onInviteJoinLiveResponse(int seq, String fromUserID, String fromUserName, String roomID) {
                Toast.makeText(AnchorVideoUI.this, "有连麦请求，来自房间："+roomID+"；用户id："+fromUserID+"；昵称:"+fromUserName, Toast.LENGTH_LONG).show();
            }

            @Override
            public void removeRenderViewByStreamDelete(ZegoStreamInfo streamInfo) {
                AnchorVideoUI.this.mVideoLayoutModel.removeStreamToViewInLayout(streamInfo.streamID);
                AnchorVideoUI.this.playStreamids.remove(streamInfo.streamID);
            }
        ...
        try {
            ...
            if(!isResponse){
                ...
                TextureView localPreviewView = AnchorVideoUI.this.mVideoLayoutModel.addStreamToViewInLayout(AnchorVideoUI.this.mPublishStreamid);
    ...
    public static void actionStart(Activity activity, String roomID) {
        Intent intent = new Intent(activity, AnchorVideoUI.class);
